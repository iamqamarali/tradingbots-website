/**
 * Scripts Page - Bot Management JavaScript
 * Handles all UI interactions and API communication for bot scripts
 */
...
function setupEventListeners() {
    ...
    if (elements.autoRestartCheckbox) {
    ...
    if (scripts.length === 0) {
        elements.scriptsList.innerHTML = `
            <div class="empty-scripts">
                ...
                <p>No scripts yet.<br>Click + to add one.</p>
            </div>
        `;
        return;
    }
...
function isNearBottom() {
    if (!elements.logsContent) return true;
...
async function fetchLogs(forceFullRefresh = false) {
    if (!currentScript || !elements.logsContent) return;
...
function updateStats() {
    ...
    // Update welcome content based on whether there are scripts
    if (elements.welcomeTitle && elements.welcomeDescription && elements.welcomeBtnText) {
        if (scripts.length > 0) {
            elements.welcomeTitle.textContent = 'Script Manager';
            elements.welcomeDescription.textContent = 'Select a script from the sidebar to view its code and logs, or add a new one.';
            elements.welcomeBtnText.textContent = 'Add New Script';
        } else {
            elements.welcomeTitle.textContent = 'Script Manager';
            elements.welcomeDescription.textContent = 'Create and manage your trading bot scripts. Upload, edit, and run your Python trading bots.';
            elements.welcomeBtnText.textContent = 'Add Your First Script';
        }
    }
}
